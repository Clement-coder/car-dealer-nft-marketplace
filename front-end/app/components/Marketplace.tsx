...
import { DollarSign, ShoppingCart, Trash2, XCircle, Search, BadgeCheck } from 'lucide-react';
    ...
    <div className="min-h-screen pt-24 md:pt-20 px-4 sm:px-6 lg:px-8 pb-12">
      <div className="max-w-7xl mx-auto">
        <div className="mb-10">
          ...
          <Search className='relative  top-9 left-3 z-40'/>
          <input
            type="text"
            placeholder="Search by name, model, or status..."
            className="w-full bg-white/10 backdrop-blur-lg rounded-lg px-12 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500"
            value={searchQuery}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCars.map((car) => (
            <div
              key={car.id}
              className="bg-white/10 backdrop-blur-lg rounded-2xl overflow-hidden shadow-xl border border-white/20 transition-all ease-in-out duration-500 hover:scale-105"
            >
              <div className="relative h-48 overflow-hidden">
                <img src={car.image} alt={car.name} className="w-full h-full object-cover" />
                <div className="absolute top-3 right-3 bg-gradient-to-r from-indigo-600/40 to-cyan-500/40 border border-cyan-500 px-3  rounded-full">
                  <span className="text-white text-sm font-bold">{car.year}</span>
                </div>
                <div
                  className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-bold ${
                    car.status === CarStatus.SOLD
                      ? 'bg-red-600/40 border border-red-300 text-white'
                      : car.status === CarStatus.PENDING
                      ? 'bg-yellow-500/40 border border-yellow-300 text-white'
                      : 'bg-green-500/40 border border-green-300 text-white'
                  }`}
                >
                  {car.status}
                </div>
              ...
              <div className="p-6">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`flex-1 py-3 rounded-lg flex items-center justify-center font-bold transition-all ${
                      car.status === CarStatus.SOLD
                        ...
                        : 'bg-gradient-to-r from-green-500/40 to-teal-500/30 border border-teal-500 hover:from-green-600/30 hover:to-teal-600/30 cursor-pointer ease-in-out duration-300 transition-transform text-white'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className="p-3 cursor-pointer bg-red-600/40 border border-red-300 text-white rounded-lg flex items-center justify-center transition-all"
                  >
            ...
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 max-w-sm w-full text-center text-white shadow-2xl">
              ...
              <div className="flex justify-center gap-4">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-lg bg-green-600/40 hover:bg-green-700/40 border border-green-300 transition-all"
                >
                  Confirm Buy
                </button>
              </div>
